...
pub mod tip_board {
    ...
    pub fn add_tip(ctx: Context<AddTipContext>, amount: u64, timestamp: i64, nft_mint: String) -> Result<()> {
        ...
        // filter for the tip with the same tipper, if it exists add the amount to the existing tip, else create a new tip
        let mut current_tips = ctx.accounts.tipboard.tips.clone();
        ...
        let mut tip_exists = false;

        for tip in current_tips.iter_mut() {
            if tip.tipper == new_tip.tipper {
                msg!("Tip already exists, adding to the existing tip {0}", tip.amount);

                tip.amount += new_tip.amount;
                tip.timestamp = new_tip.timestamp;
                tip.nft_mint = new_tip.nft_mint.clone();
                tip_exists = true;

                msg!("Tip amount after adding {0}", tip.amount);
                break;
            }
        }

        if !tip_exists {
            current_tips.push(new_tip);
        }

        // sort the tips by amount
        current_tips.sort_by(|a, b| b.amount.cmp(&a.amount));

        ctx.accounts.tipboard.tips = current_tips;
